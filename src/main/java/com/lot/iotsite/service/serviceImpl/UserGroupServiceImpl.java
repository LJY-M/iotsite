...
public class UserGroupServiceImpl implements UserGroupService{
    ...
    public Boolean save(UserGroup userGroup){
        QueryWrapper<UserGroup> queryWrapper=new QueryWrapper<>();
        queryWrapper.eq(UserGroup.ISLEADER, userGroup.getIsleader());
    ...
    public List<UserDto> getMember(Long id){
        QueryWrapper<UserGroup> queryWrapper=new QueryWrapper<>();
        queryWrapper.eq(UserGroup.GROUPID, id);
        queryWrapper.orderByAsc(UserGroup.USERID);
    ...
    public List<UserDto> getLeader(Long id){
        QueryWrapper<UserGroup> queryWrapper=new QueryWrapper<>();
        queryWrapper.eq(UserGroup.GROUPID, id);
        queryWrapper.eq(UserGroup.ISLEADER,1);
        queryWrapper.orderByAsc(UserGroup.USERID);
        // 在UserGroup表中查询到记录相应小组的所有成员记录
        List<UserGroup> userGroups = userGroupMapper.selectList(queryWrapper);
        List<UserDto> userDtos = new ArrayList<>();
